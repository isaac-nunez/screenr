...
library(ggplot2); library(shiny); library(tidyverse); library(vroom)
...
tamizajes_metabolicos <- function(diabetes, edad, sexo, cv, imc, has, tabaco, tab){
  metabolicos <- tibble(enfermedad = c("-Diabetes mellitus tipo 2 (comorbilidades): ", 
                        ...
                        indicacion = c(#Diabetes mellitus tipo 2 (comorbilidades)
                        case_when(
                        diabetes == "No"~"",
                        ...
                        case_when(diabetes == "No"~"",#Diabetes mellitus tipo 2 (tamizajes clínicos)
                        ...
                        case_when(diabetes == "No"~"",#Diabetes mellitus tipo 2 (tamizajes de laboratorio)
                        ...
                        case_when(diabetes == "No"~"",#Diabetes mellitus tipo 2 (prevención CV primaria)
                                  diabetes == "Sí"&cv =="Sí"~"",
                        ...
                        case_when(diabetes == "No"~"",#Diabetes mellitus tipo 2 (prevención CV secundaria)
                                  diabetes == "Sí"&cv =="No"~"",
                        ...
                        case_when(diabetes == "Sí"|cv == "Sí" ~"",#Riesgo cardiovascular
                        ...
                        case_when(imc>25&imc<35~"Cambios en estilo de vida, considerar enviar a nutrición clínica",#Sobrepeso/Obesidad
                                  imc>35~"Considerar enviar a clínica de obesidad",
                                  imc<25~""),
                        case_when(tabaco == "Sí"~"Medidas para suspender tabaco, considerar enviar a clínica",#Tabaquismo
                                  tabaco == "No"~""),
                        case_when(tabaco == "Sí"&edad >=65&edad<=75&sexo=="Masculino"~"Tamizaje con US abdominal una vez en la vida",
                                  tabaco == "No"&edad >=65&edad<=75&sexo=="Masculino"~"Valorar tamizaje considerando antecedente 
                                  familiar de aneurisma y riesgo cardiovascular")
                        ),
                        referencia = c(rep("Standards of Medical Care in Diabetes 2021. Diabetes Care 2021;44(Suppl. 1):S40–S52 y S125-S150.
                                       ...
                                       rep("2019 ACC/AHA Guideline on the Primary Prevention of Cardiovascular Disease. Circulation. 2019;140:e596–e646.
                                           Disponible en: https://www.ahajournals.org/doi/10.1161/CIR.0000000000000678", times = 3),
                                       "Screening for Abdominal Aortic Aneurysm. USPSTF Recommendation Statement.
                                       JAMA 2019;322(22):2211-2218. doi:10.1001/jama.2019.18928.")) %>% filter(indicacion!="")
  
  if_else(tab==1,
  paste(str_c(metabolicos$enfermedad, metabolicos$indicacion), collapse = "\n"),
  paste(str_c(metabolicos$enfermedad, metabolicos$referencia), collapse = "\n"))
}
...
ui <- fluidPage(
  ...
  "Únicamente llena el panel de la izquierda y da clic en 'Aplicar' para obtener los tamizajes correspondientes.",
  sidebarLayout(
    sidebarPanel(
  numericInput("edad", "Edad", 18, min = 18, max = 120, width  = "100px"),
  selectInput("sexo", "Sexo biológico", choices = c("Femenino", "Masculino"), width  = "110px"),
  numericInput("peso", "Peso (kg)", 70, min = 20, max = 500, width  = "100px"),
  numericInput("talla", "Talla (cm)", 160, min = 30, max = 250, width  = "100px"),
  radioButtons("dm2", "Diabetes mellitus tipo 2", choices = c("Sí", "No"), width  = "100px"),
  radioButtons("has", "Hipertensión", choices = c("Sí", "No"), width  = "100px"),
  radioButtons("cv", "Antecedente de evento cardiovascular", choices = c("Sí", "No"), width  = "100px"),
  radioButtons("tabaco", "Antecedente de tabaquismo", choices = c("Sí", "No"), width  = "100px"),
  ...
  mainPanel(tabsetPanel(type = "tabs",
    ...
    tabPanel(title="Encuesta",
             br(),
             br(),
             br(),
             "Aplicación escrita por Isaac Núñez y Adrián Soto-Mota (octubre 2021).",
             "Gracias por tu interés en nuestra aplicación.",
             br(),
             "Al ser esta la versión beta, queremos encontrar todos los errores, recibir todas las sugerencias posibles y hacer cambios para mejorar la aplicación.",
             br(),
             "Se espera que la versión final incluya funcionalidad para servicios específicos (ej.infectología, hematología, UTI, etc).",
             br(),
             br(),
             "Si tienes un minuto, por favor responde ",
             tags$a(href="https://forms.gle/Aww5Z74uhmtuqfrs9", 
                    "nuestra encuesta sobre como mejorar la aplicación." ),
             br(),
             )), width = 10)))
